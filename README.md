...
The script should be copied over into one cell, but requires three inputs: client ID, client secret, and redirect URI.
    Create an app on Spotify's developer dashboard to get client ID and secret.
    Use any HTTPS for redirect URI as long as the script's matches the dev dashboard's.
    
Uncomment first line if first run in session.
